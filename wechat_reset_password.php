<?php
/**
 * Password Reset Page — WeChat-Only Mode (SOAP)
 *
 * This page allows a WeChat-authenticated user to reset their game account
 * password. A new random password is generated and set via SOAP.
 *
 * Flow:
 *   1. User must be logged in via WeChat (session check)
...
// User must be logged in via WeChat
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['confirm_reset'])) {
    $result = WeChatAuth::resetMyPassword();
    ...
    if ($result['success']) {
        ...
        if ($msg === 'account_not_found') {
            ...
            $errorMsg = lang('not_logged_in') ?: '请先登录微信账号。';
    ...
    <div class="reset-body">
        ...
        <?php if ($showResult): ?>
            ...
            <div style="text-align: center;">
                <div style="font-size: 48px; color: var(--wechat-green); margin-bottom: 10px;">
                    <i class="fas fa-check-circle"></i>
                </div>
                <h4 style="color: var(--wechat-green); margin-bottom: 20px;">
